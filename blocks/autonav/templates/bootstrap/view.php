...
foreach ($navItems as $ni) {
	...
	*/
	...
	if ($ni->hasSubmenu) {
        ...
        if($ni->level == 1) {
            $ni->caret = '<span class="caret"></span>';
        }else{
	...
	*/
	...
	$ni->classesLi = implode(" ", $classesLi);
	$ni->classesA = implode(" ", $classesA);
	$ni->classesUl = implode(" ", $classesUl);
	$ni->attributesA = implode(" ", $attributesA);
}
...
echo '<ul class="nav navbar-nav navbar-right">'; //opens the top-level menu
	...
	foreach ($navItems as $ni) {
		...
		echo '<li class="' . $ni->classesLi . '">'; //opens a nav item

		echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classesA . '" '.$ni->attributesA.'>'. $ni->name . $ni->caret. '</a>';

		if ($ni->hasSubmenu) {
			echo '<ul class="' . $ni->classesUl . '">'; //opens a dropdown sub-menu
		} else {
